...
def solver_eigs(A, N_eigs, guess_value=0, guess_vector=None, timing=False):
    # solves for the eigenmodes of A

    if timing:
        start = time()
    (values, vectors) = spl.eigs(A, k=N_eigs, sigma=guess_value, v0=guess_vector, which='LM')
    if timing:
        end = time()
        print('Elapsed time for spl.eigs() is %.4f secs' % (end - start))
...
def solver_complex2real(A11, A12, b, timing=False, solver=DEFAULT_SOLVER):
    ...
    if solver.lower() == 'pardiso':
        ...
        pSolve = pardisoSolver(Areal, mtype=11)
        pSolve.factor()
